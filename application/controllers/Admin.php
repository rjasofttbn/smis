...
class Admin extends CI_Controller {
    ...
    //12-01-2019
    function teacher_information($teacher_id = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');

        $page_data['page_name'] = 'teacher_information';
        $page_data['page_title'] = get_phrase('teacher_information') . " - " . get_phrase('teacher') . " : " .
                $this->crud_model->get_class_name($teacher_id);
...
function manage_attendance_teacher() {
    if ($this->session->userdata('admin_login') != 1)
        redirect(base_url(), 'refresh');

    $page_data['page_name'] = 'manage_attendance_teacher';
    $page_data['page_title'] = get_phrase('manage_teachers_attendance');
    $this->load->view('backend/index', $page_data);
}
...
//    exit;
    ...
    redirect(base_url() . 'index.php?admin/manage_attendance_view_teacher/' . $data['timestamp'], 'refresh');
}

function manage_attendance_view_teacher($timestamp = '') {
    if ($this->session->userdata('admin_login') != 1)
        redirect(base_url(), 'refresh');
    $page_data['timestamp'] = $timestamp;
    $page_data['page_name'] = 'manage_attendance_view_teacher';
    $page_data['page_title'] = get_phrase('manag_teacheres_attendance');
    $this->load->view('backend/index', $page_data);
}

function attendance_update_teacher($timestamp = '') {
    $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
//    $attendance_of_teachers = $this->db->get_where('teacher_attendance')->result_array();
    ...
    foreach ($attendance_of_teachers as $row) {
        $attendance_status = $this->input->post('status_' . $row['teacher_attendance_id']);
        $this->db->where('teacher_attendance_id', $row['teacher_attendance_id']);
        $this->db->update('teacher_attendance', array('status' => $attendance_status));
    }

    $this->session->set_flashdata('flash_message', get_phrase('attendance_update_teacher'));
    redirect(base_url() . 'index.php?admin/manage_attendance_view_teacher/' . $timestamp, 'refresh');
}
...
function attendance_report_teacher() {
    $page_data['month'] = date('m');
    $page_data['page_name'] = 'attendance_report_teacher';
    $page_data['page_title'] = get_phrase('attendance_report_teacher');
    $this->load->view('backend/index', $page_data);
}

function attendance_report_teacher_selector() {
    if ($this->input->post('sessional_year') == '') {
//        $this->session->set_flashdata('error_message', get_phrase('please_make_sure_class_and_sessional_year_are_selected'));
        redirect(base_url() . 'index.php?admin/attendance_report_teacher', 'refresh');
    }
...
//    $data['section_id'] = $this->input->post('section_id');
    $data['month'] = $this->input->post('month');
    $data['sessional_year'] = $this->input->post('sessional_year');
    redirect(base_url() . 'index.php?admin/attendance_report_view_teacher/' . $data['month'] . '/' . $data['sessional_year'], 'refresh');
}

function attendance_report_view_teacher($month = '', $sessional_year = '') {
    if ($this->session->userdata('admin_login') != 1)
        redirect(base_url(), 'refresh');
...
//    $page_data['section_id'] = $section_id;
    $page_data['month'] = $month;
    $page_data['sessional_year'] = $sessional_year;
    $page_data['page_name'] = 'attendance_report_view_teacher';
    $page_data['page_title'] = get_phrase('attendance_report_of_teacher');
    $this->load->view('backend/index', $page_data);
}

function attendance_report_print_view_teacher($month = '', $sessional_year = '') {
    if ($this->session->userdata('admin_login') != 1)
        redirect(base_url(), 'refresh');
    ...
    $page_data['month'] = $month;
    $page_data['sessional_year'] = $sessional_year;
    $this->load->view('backend/admin/attendance_report_print_view_teacher', $page_data);
}
